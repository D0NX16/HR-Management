...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => cb(null, 'AppliedVisit-' + Date.now() + path.extname(file.originalname))
});
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await connection.query('SELECT * FROM applied_visits ORDER BY id DESC');
        res.json(rows);
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        connection = await pool.getConnection();
        const sql = `INSERT INTO applied_visits 
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        connection = await pool.getConnection();
        const sql = `UPDATE applied_visits SET 
            visit_date=?, institution_name=?, city=?, cp_name=?, designation=?, 
            cp_mobile_no=?, cp_whatsapp=?, email_id=?, award_category=?, ide=?, remarks=? 
            WHERE id=?`;
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        connection = await pool.getConnection();
        await connection.query('DELETE FROM applied_visits WHERE id = ?', [req.params.id]);
